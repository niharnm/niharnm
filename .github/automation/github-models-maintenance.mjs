...
function parseTargets() {
  const rawTargets = process.env.TARGET_REPOS || '';
  return rawTargets
    .split(/[\n,]/)
    .map((repo) => repo.trim())
...
function rotatedTargets(targets) {
  ...
  return rotated.sort((a, b) => {
    if (a === process.env.GITHUB_REPOSITORY) return 1;
    if (b === process.env.GITHUB_REPOSITORY) return -1;
...
async function writeResult(markdown) {
  ...
  if (resultFile) {
    await writeFile(resultFile, `${markdown}\n`, 'utf8');
  }
...
function summarizePackageJson(repoDir) {
  const packagePath = path.join(repoDir, 'package.json');
  if (!existsSync(packagePath)) return '';
  try {
    const parsed = JSON.parse(run('cat', ['package.json'], { cwd: repoDir }).stdout);
    const useful = {
      name: parsed.name,
...
async function askGithubModels({ repo, readmePath, readme, files, packageJson, recentCommits, policy }) {
  const prompt = [
    ...
    'Set should_change to false if there is no concrete, useful README improvement. Only edit the candidate README file. Preserve correct existing information. Do not add timestamps, badges, fake features, generated-by text, automation notes, or filler.',
  ].filter(Boolean).join('\n\n');
...
async function tryRepo(repo, workDir, policy) {
  ...
  const branch = run('git', ['branch', '--show-current'], { cwd: repoDir }).stdout.trim();
  const recentCommits = run('git', ['log', '--oneline', '-n', '8'], { cwd: repoDir }).stdout.trim();
  const { readmePath, files } = findReadme(repoDir);

  if (!readmePath) {
    return { changed: false, reason: 'No README.md candidate found.' };
  }

  const original = await readFile(path.join(repoDir, readmePath), 'utf8');
  if (original.length > 18000) {
    return { changed: false, reason: `${readmePath} is too large for safe free-model replacement.` };
  }

  const packageJson = summarizePackageJson(repoDir);
  const proposal = await askGithubModels({
    repo,
    readmePath,
    readme: original,
    files,
  ...
  if (!proposal.should_change) {
    return { changed: false, reason: proposal.summary || 'Model found no useful README change.' };
  }
  ...
  if (!replacement.trim()) {
    return { changed: false, reason: 'Model returned an empty replacement.' };
  }
  ...
  await writeFile(path.join(repoDir, readmePath), replacement, 'utf8');
  const diff = run('git', ['diff', '--', readmePath], { cwd: repoDir }).stdout;
  if (!diff.trim()) {
    return { changed: false, reason: 'Replacement produced no diff.' };
  }
  ...
  if (!allowDirectCommits) {
    ...
    run('gh', ['pr', 'create', '--repo', repo, '--base', branch, '--head', branchName, '--title', commitMessage, '--body', prBody], { cwd: repoDir });
    const prUrl = run('gh', ['pr', 'view', '--repo', repo, branchName, '--json', 'url', '--jq', '.url'], { cwd: repoDir }).stdout.trim();
  ...
  try {
    ...
    const branchName = `automation/github-models-${Date.now()}`;
    run('git', ['push', '-u', 'origin', `HEAD:${branchName}`], { cwd: repoDir });
